...
public class QueueByTwoStacks {
	...
	public SinglyLinkedNode<UUID, Integer> dequeue() {
		...
		if (dequeuingStack.isEmpty()) {
			shiftStack(enqueuingStack, dequeuingStack);
		}
		return dequeuingStack.pop();
	}

	private void shiftStack(Stack<SinglyLinkedNode<UUID, Integer>> stackFrom, Stack<SinglyLinkedNode<UUID, Integer>> stackTo) {
