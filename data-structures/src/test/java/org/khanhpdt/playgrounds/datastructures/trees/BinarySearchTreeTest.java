...
public class BinarySearchTreeTest {
	...
	private List<BinaryTreeNode> defaultNodes;
	private BinarySearchTree defaultBST;
	...
	private List<BinaryTreeNode> getDefaultNodes() {
		return Arrays.asList(30, 20, 25, 35, 15, 27, 40, 23)
				.stream()
				.map(BinaryTreeNode::new)
				.collect(Collectors.toList());
	}

	private BinarySearchTree getDefaultTree(List<BinaryTreeNode> defaultNodes) {
	...
	public void testInsertFirstNode() throws Exception {
		...
		BinaryTreeNode firstNode = BinaryTreeNode.random();
	...
	public void testTraverseInOrderIterative() throws Exception {
		List<BinaryTreeNode> inOrderNodes = defaultBST.traverseInOrderIterative();
	...
	public void testTraverseInOrderRecursive() throws Exception {
		List<BinaryTreeNode> inOrderNodes = defaultBST.traverseInOrderRecursive();
		IntStream.range(0, defaultNodes.size())
				.forEach(i -> assertThat(inOrderNodes.get(i), is(defaultNodes.get(IN_ORDER_INDEXES.get(i)))));
	}
	...
	public void testTraversePreOrderRecursive() throws Exception {
		List<BinaryTreeNode> preOrderNodes = defaultBST.traversePreOrderRecursive();
		IntStream.range(0, defaultNodes.size())
	...
	public void testTraversePreOrderIterative() throws Exception {
		List<BinaryTreeNode> preOrderNodes = defaultBST.traversePreOrderIterative();
		IntStream.range(0, defaultNodes.size())
				.forEach(i -> assertThat(preOrderNodes.get(i), is(defaultNodes.get(PRE_ORDER_INDEXES.get(i)))));
	}
	...
	public void testTraversePostOrderRecursive() throws Exception {
		List<BinaryTreeNode> postOrderNodes = defaultBST.traversePostOrderRecursive();
		IntStream.range(0, defaultNodes.size())
	...
	public void testTraversePostOrderIterative() throws Exception {
		List<BinaryTreeNode> postOrderNodes = defaultBST.traversePostOrderIterative();
		IntStream.range(0, defaultNodes.size())
				.forEach(i -> assertThat(postOrderNodes.get(i), is(defaultNodes.get(POST_ORDER_INDEXES.get(i)))));
	}
