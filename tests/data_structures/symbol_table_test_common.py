...
class SymbolTableTestCommon:
    ...
    def test_size_without_lower_bound(self):
        assert self.table.size(high=Key(12)) == 0
    ...
    def test_size_without_higher_bound(self):
        assert self.table.size(high=Key(12)) == 0
    ...
    def test_size_with_same_bounds(self):
        self.put_default_items()
        assert self.table.size(low=Key(7), high=Key(7)) == 0
        assert self.table.size(low=Key(8), high=Key(8)) == 1

    def test_size_with_invalid_bounds(self):
        self.put_default_items()
